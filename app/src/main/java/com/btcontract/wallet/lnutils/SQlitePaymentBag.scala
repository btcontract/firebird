...
class SQlitePaymentBag(db: SQLiteInterface) extends PaymentBag with PaymentUpdaterToSuccess {
  ...
  def updOkOutgoing(upd: UpdateFulfillHtlc, sent: MilliSatoshi, fee: MilliSatoshi): Unit =
    db.change(PaymentTable.updOkOutgoingSql, PaymentStatus.SUCCEEDED, upd.paymentPreimage.toHex,
      sent.toLong: java.lang.Long, fee.toLong: java.lang.Long, upd.paymentHash.toHex)

  def updOkIncoming(add: UpdateAddHtlc): Unit = db.change(PaymentTable.updOkIncomingSql, PaymentStatus.SUCCEEDED, add.amountMsat.toLong: java.lang.Long, System.currentTimeMillis: java.lang.Long, add.paymentHash.toHex)

  def abortPayment(paymentHash: ByteVector32): Unit = db.change(PaymentTable.updStatusSql, PaymentStatus.ABORTED, paymentHash.toHex, PaymentStatus.SUCCEEDED)

  def addOutgoingPayment(nodeId: PublicKey, prex: PaymentRequestExt, description: PaymentDescription, action: Option[PaymentAction], finalAmount: MilliSatoshi, balanceSnap: MilliSatoshi, fiatRateSnap: Rates): Unit =
    db.change(PaymentTable.newSql, nodeId.toString, prex.raw, ByteVector32.Zeroes.toHex, PaymentStatus.PENDING, System.currentTimeMillis: java.lang.Long, description.toJson.compactPrint,
      action.map(_.toJson.compactPrint).getOrElse(new String), prex.paymentHashStr, 0L: java.lang.Long /* RECEIVED = 0 MSAT, OUTGOING */, finalAmount.toLong: java.lang.Long,
      0L: java.lang.Long /* FEE IS UNCERTAIN YET */, balanceSnap.toLong: java.lang.Long, fiatRateSnap.toJson.compactPrint,
      0: java.lang.Integer /* INCOMING = 0 */, new String /* EMPTY EXT FOR NOW */)

  def addIncomingPayment(prex: PaymentRequestExt, preimage: ByteVector32, description: PaymentDescription, finalAmount: MilliSatoshi, balanceSnap: MilliSatoshi, fiatRateSnap: Rates): Unit =
    db.change(PaymentTable.newSql, invalidPubKey.toString, prex.raw, preimage.toHex, PaymentStatus.PENDING, System.currentTimeMillis: java.lang.Long, description, new String /* NO ACTION */,
      prex.paymentHashStr, finalAmount.toLong: java.lang.Long, 0L: java.lang.Long /* SENT = 0 MSAT, INCOMING */, 0L: java.lang.Long /* NO FEE FOR INCOMING */,
      balanceSnap.toLong: java.lang.Long, fiatRateSnap.toJson.compactPrint, 1: java.lang.Integer /* INCOMING = 1 */, new String /* EMPTY EXT FOR NOW */)

  def toNodeSummary(nodeId: PublicKey): Try[SentToNodeSummary] = db.select(PaymentTable.selectToNodeSummarySql, nodeId.toString, PaymentStatus.SUCCEEDED) headTry { rc =>
    SentToNodeSummary(fees = MilliSatoshi(rc.c getLong 0), sent = MilliSatoshi(rc.c getLong 1), count = rc.c getLong 2)
  }

  def betweenSummary(from: Long, to: Long): Try[TotalStatSummary] = db.select(PaymentTable.selectBetweenSummarySql, from.toString, to.toString, PaymentStatus.SUCCEEDED) headTry { rc =>
