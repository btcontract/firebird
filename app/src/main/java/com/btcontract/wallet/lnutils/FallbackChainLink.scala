...
class FallbackChainLink extends ChainLink { me =>
  ...
  private[this] def processEvidence(height: Int): Unit = if (chainTipCanBeTrusted) runAnd(stop) { for (lst <- listeners) lst.onChainTipConfirmed }

  def blockChain: Observable[Int] = Rx.retry(Rx.ioQueue.map(_ => to[BlockChainHeight](get("https://blockchain.info/latestblock").body).height), Rx.pickInc, times = 2 to 6 by 2)
  def blockCypher: Observable[Int] = Rx.retry(Rx.ioQueue.map(_ => to[BlockCypherHeight](get("https://api.blockcypher.com/v1/btc/main").body).height), Rx.pickInc, times = 2 to 6 by 2)
  def blockStream: Observable[Int] = Rx.retry(Rx.ioQueue.map(_ => get("https://blockstream.info/api/blocks/tip/height").body.toInt), Rx.pickInc, times = 2 to 6 by 2)
